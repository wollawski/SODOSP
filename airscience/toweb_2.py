...
with col1:
    ...
    if best_combo:
        ...
        st.markdown(f"""
        **选型核心数据对比：**
        * 目标总风量需求：`{round(total_airflow, 0)} m³/h`
        * 组合总额定风量：`{total_combo_airflow} m³/h` 
...
if best_combo:
    ...
    if st.button("开始一键生成机房 3D 布局", type="primary"):
        with st.spinner("正在进行空间压缩寻优与正交管线寻路，请稍候（约 3-8 秒）..."):
            ...
            try:
                ...
                m3.metric("机房纯占地面积", f"{layout_2d['polygon_area']:.2f} m²")
